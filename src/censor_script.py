...
def censor_media(model_type: str, input_media: str, blocked_words: list, beep_file: str = None, progress_callback=None):
    ...
    KEEP_START_FRACTION = 0.25
    KEEP_END_FRACTION = 0.0
    ...
    for segment in result["segments"]:
        for word in segment["words"]:
            ...
            if clean_word in blocked_words:
                ...
                mute_ranges.append((start, end, clean_word))
    ...
    volume_expr = " + ".join([f"between(t,{start},{end})" for start, end, _word in mute_ranges])
    ...
    if use_beep:
        ...
        # (looped/trimmed to exactly fill each muted window) during those same ranges.
        filter_parts = [
            f"[0:a]{volume_filter},aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo[muted]"
        ]
        mix_inputs = ["[muted]"]
        for i, (start, end, _word) in enumerate(mute_ranges):
